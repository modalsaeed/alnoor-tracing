...
class MedicalCentreDialog(QDialog):
    ...
    def validate_input(self) -> tuple[bool, str]:
        ...
        if not name:
            return False, "Medical centre name is required."
        
        if not reference:
            return False, "Medical centre reference is required."
        
        if len(name) < 2:
            return False, "Medical centre name must be at least 2 characters."
        
        if len(reference) < 2:
            return False, "Medical centre reference must be at least 2 characters."
        
        # Check for duplicate reference
        if not self.is_edit_mode or (self.centre and reference.upper() != self.centre.reference):

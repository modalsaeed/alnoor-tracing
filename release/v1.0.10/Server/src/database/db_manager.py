...
def get_database_instance(db_path: Optional[str] = None):
    ...
    """
    # Setup debug log file in a writable location
    try:
        if getattr(sys, 'frozen', False):
            # Use LocalAppData for installed apps (always writable)
        ...
        debug_log = log_dir / 'connection_debug.log'
        ...
        import tempfile
        debug_log = Path(tempfile.gettempdir()) / 'alnoor_connection_debug.log'
        with open(debug_log, 'a', encoding='utf-8') as f:
            f.write(f"\n{'='*70}\n")
            f.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | APPLICATION STARTED (FALLBACK LOCATION)\n")
            f.write(f"ERROR with primary location: {e}\n")
            f.write(f"{'='*70}\n")
    ...
    # Always log where we're writing
    log_debug(f"📝 Debug log location: {debug_log.absolute()}")

...
class DeliveryNote(Base):
    """Delivery notes generated from coupons, for persistent tracking and reporting."""
    __tablename__ = 'delivery_notes'
    id = Column(Integer, primary_key=True, autoincrement=True)
    delivery_note_number = Column(String(100), unique=True, nullable=False, index=True)
    centre_id = Column(Integer, ForeignKey('medical_centres.id'), nullable=False)
    centre_name = Column(String(255), nullable=False)
    location_id = Column(Integer, ForeignKey('distribution_locations.id'), nullable=True)
    ...
    total_cartons = Column(Integer, nullable=False)
    date_created = Column(DateTime, default=datetime.utcnow, nullable=False)
    file_path = Column(String(1024), nullable=True)
    # Relationships
    centre = relationship('MedicalCentre')
    location = relationship('DistributionLocation')
    product = relationship('Product')
    po = relationship('PurchaseOrder')
    # Optionally, you could add a relationship to coupons if you add a delivery_note_id to PatientCoupon
    def __repr__(self):
        return f"<DeliveryNote(id={self.id}, number='{self.delivery_note_number}', centre='{self.centre_name}', product='{self.product_name}', date='{self.date_created}')>"

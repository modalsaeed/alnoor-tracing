...
class DatabaseClient:
    def create(self, model_class, data):
        """
        Generic create method for widget compatibility. Routes to model-specific API endpoints.
        Converts datetime values to ISO strings for JSON serialization.
        Args:
            model_class: The ORM model class (e.g., DeliveryNote)
            data: Dictionary of fields to create
        Returns:
            The created record as a dict (API response)
        """
        from .models import Product, PurchaseOrder, Purchase, Pharmacy, DistributionLocation, MedicalCentre, PatientCoupon, Transaction, DeliveryNote
        ...
        if model_class == DeliveryNote:
            return self.create_delivery_note(**data)
        elif model_class == Product:
            return self.create_product(**data)
        elif model_class == PurchaseOrder:
            return self.create_purchase_order(**data)
        elif model_class == Purchase:
            return self.create_purchase(**data)
        elif model_class == Pharmacy:
            return self.create_pharmacy(**data)
        elif model_class == DistributionLocation:
            return self.create_distribution_location(**data)
        elif model_class == MedicalCentre:
            return self.create_medical_centre(**data)
        elif model_class == PatientCoupon:
            return self.create_patient_coupon(**data)
        elif model_class == Transaction:
            return self.create_transaction(**data)
        else:
            raise NotImplementedError(f"create not implemented for {getattr(model_class, '__name__', str(model_class))}")
    ...
    def get_all(self, model_class):
        ...
        from .models import (Product, PurchaseOrder, Purchase, Pharmacy, DistributionLocation, 
                     MedicalCentre, PatientCoupon, Transaction, DeliveryNote)
        ...
        model_map = {
            ...
            Transaction: self.get_all_transactions,
                DeliveryNote: self.get_all_delivery_notes,
            }
        ...
        if method:
            ...
            raise NotImplementedError(f"get_all not implemented for {getattr(model_class, '__name__', str(model_class))}")
    ...
    def update(self, record):
        ...
        if hasattr(record, '__tablename__'):
        ...
        elif hasattr(record, '__class__') and hasattr(record.__class__, '__name__') and not isinstance(record, dict):
            ...
            if 'unit' in record and 'reference' in record and 'name' in record:
                ...
                model_name = 'distribution_location'
            elif 'trn' in record and 'name' in record and 'contact_person' in record:
                model_name = 'pharmacy'
            elif 'address' in record and 'contact_person' in record and 'phone' in record:
                model_name = 'medical_centre'
            elif 'coupon_reference' in record and 'date_received' in record:
        ...
        raise NotImplementedError(f'Update not implemented for {model_name or type(record).__name__}')
        """
